...
(() => {
  ...
  function rawIdentity(item, context) {
    const { url } = resolveRawUrl(item, context);
    if (url) return `url:${url}`;
    const name = T.sanitizeFilename(item?.name || "attachment").toLowerCase();
    return `meta:${item?.messageN ?? "?"}:${name}:${String(item?.size || "")}`;
  }
  ...
  function mergeRaw(primary, supplemental, context) {
    ...
    for (const item of out) {
      const key = rawIdentity(item, context);
      if (key.startsWith("url:")) byUrl.set(key, item);
    }

    let supplementalOnly = 0;
    for (const item of supplemental || []) {
      const key = rawIdentity(item, context);
      const existing = key.startsWith("url:") ? byUrl.get(key) : null;
      if (existing) {
