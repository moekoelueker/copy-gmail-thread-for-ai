...
(() => {
  ...
  function parseDownloadUrl(value) {
    ...
    // Gmail percent-encodes the filename here. Left encoded, a chip name
    // sanitizes to "Claim_20form.pdf" while the print-view entry stays
    // "Claim form.pdf", so mergeRaw cannot pair them and reports one file twice.
    // Decoding is safe because normalise() still sanitizes afterwards, so a
    // decoded separator or control character never reaches a download path.
    let name = s.slice(first + 1, urlAt);
    try {
      name = decodeURIComponent(name);
    } catch (_) {
      /* a lone % is a literal, not an escape; keep the raw name */
    }
    return {
  ...
  function mergeRaw(primary, supplemental, context) {
    ...
    for (const item of supplemental || []) {
      ...
      if (existing) {
        ...
        const existingUrl = resolveRawUrl(existing, context).url;
        if (url && !existingUrl) {
          existing.href = url;
        } else if (!existingUrl && !existing.href) {
          // The chip carried a URL the policy refused. Carry it across anyway
          // so the merged entry reports a refusal rather than claiming no link
          // was ever found. It stays unusable — normalise rejects it again, no
          // download can reference it, and raw URLs never reach the output.
          const refused = rawUrls(item).values[0];
          if (refused) existing.href = refused;
        }
        continue;
      }
      out.push(item);
