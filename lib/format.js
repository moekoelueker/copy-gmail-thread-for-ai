...
(() => {
  ...
  // Metadata elements — subject, recipients, participants — get full escaping.
  // Their content is short and structural, and a recipient rendered as
  // "Moe <moe@x.com>" would otherwise put a raw opening tag inside the
  // document. Bodies deliberately use the lighter escBody instead, so email
  // discussing HTML or containing code is not mangled.
  function escText(s) {
  ...
  function messageBlock(m) {
    ...
    const out = [`${head.join(" ")}>`];
    if (m.to && m.to.length) out.push(`<to>${escText(m.to.join(", "))}</to>`);
    if (m.cc && m.cc.length) out.push(`<cc>${escText(m.cc.join(", "))}</cc>`);
  ...
  function build(thread) {
    ...
    if (people.size) {
      out.push(`<participants>${escText(Array.from(people.values()).join("; "))}</participants>`);
    }
    ...
    out.push(`<source>${escText(thread.source || "print-view")}</source>`);
  ...
  CT.format = { build, escBody, escAttr, escText };
})();
