...
(() => {
  ...
  function trimQuotedText(body) {
    ...
    const beforePlaceholders = out;
    out = out.replace(PLACEHOLDERS, "");
    const elided = out !== beforePlaceholders;
    out = out.replace(/\n{3,}/g, "\n\n").trimEnd();

    // Trimming must never turn a non-empty body into an empty one — except
    // when the only thing removed was Gmail's own elision marker. There the
    // guard used to restore the marker, publishing interface text as the
    // sender's words. An empty body plus an explicit flag is the honest
    // result: Gmail withheld the content, so there is nothing to report.
    if (!out.trim()) {
      return elided
        ? { text: "", trimmed: true, elided: true }
        : { text: original, trimmed: false };
    }
